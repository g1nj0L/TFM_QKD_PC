...
All prior probabilities become p_j = 1/2.
    ...
    ┌───────────────────────────────────────┐
====│  NOTICE: in this version, I'll code   │
    │  a new function to implement the de-  │
    │  polarizing channel in the SSP-modi-  │
====│  fied version of the original code.   │
    └───┬───────────────────────────────────┘
        │        
        └> The changes wrt the original code will be marked with a [CHANGE] label
           in a comment before the change itself (also explaining it).

        └> The parts of the code I'm not sure if I should modify are marked with a
           [DOUBT] label right above the lines themselves.

        └> The old versions of the code will be left in this version commented out with
           a "OLD CODE" line just at the beginning of them.

        └> TO-DO: once all the changes are implemented, I should modify the @doc of the functions and all the other comments aswell!
        Right now they are all mostly outdated...
...
CSV.write("SSPvsBB84v2_18-03/data/SSP_depolarized_data.csv", df_export)
println()
println("SSP data successfully saved into 'SSP_depolarized_data.csv'!")