...
# [CHANGE 11/03/26]
# I've added some functions needed for the things I implemented: Plots,
# LaTeXStrings, CSV and DataFrames.
...
# [CHANGE 11/03/26]
#   Now, we have 6 states instead of 4. All the other constants remain the same
#   > const n_states = 4 --> const n_states = 6

const n_states = 6               # Number of Alice's settings: {0_Z, 1_Z, 0_X, 1_X, 0_Y, 1_Y}
const n_dim = 2                  # dim(span{|φⱼ⟩}) — SSP reference states span C²
const dim_B = 3                  # Bob's system dimension: qubit {|0⟩,|1⟩} + vacuum |vac⟩
const dim_AB = n_states * dim_B  # = 18, total dimension of ρ_AB
...
function reference_states(δ::T) where {T}
    # [CHANGE 11/03/26]
    #   We've added two additional states: |±i⟩.
    #       |±i⟩ = 1/√2 · ( |0⟩ ± i·|1⟩ )
    #   In this initial modification, we'll supose a flawless preparation of the reference
    #   states for SSP (i.e. δ = 0, the ideal case). Nonetheless, we'll still code the
    #   general scenario to ease our future steps.

    δ = 0 # Remove this line when we implement the general scenario!
    κ = 1 + δ / π
    ...
    φ_4 = [cos(κ * π / 4),  1im*sin(κ * π / 4)]                  # |+i⟩
    φ_5 = [cos(3κ * π / 4), 1im*sin(3κ * π / 4)]                 # |-i⟩
    return hcat(φ_0,φ_1,φ_2,φ_3,φ_4,φ_5)
end
...
function source_replacement_state(δ::T) where {T}
    # [DOUBT 11/03/26]
    # Now ψ will have complex elements (since φ has some complex states),
    # therefore we must change T to Complex{T}.

    φ = reference_states(δ)
    ψ = zeros(Complex{T}, n_states * n_dim) # T (BB84) -> Complex{T} (SSP)
    for j in 1:n_states
...
"""
function apply_loss_channel(ρ_AA′::AbstractMatrix, η::T) where {T} # Matrix{T} (BB84) -> AbstractMatrix (SSP)
    # [DOUBT 11/03/26]
    # Since ρ_AA′ now will be complex, we must change its data type from Matrix{T}
    # to AbstractMatrix.
...
"""
function apply_dark_count_channel(ρ_AB::AbstractMatrix, d::T) where {T} # Matrix{T} (BB84) -> AbstractMatrix (SSP)
    # [DOUBT 11/03/26]
    # Since ρ_AA′ now will be complex, we must change its data type from Matrix{T}
    # to AbstractMatrix.
...
function observed_statistics(ρ_AB::AbstractMatrix)
    # [CHANGE 11/03/26]
    # Here we are modifying Bob's POVM to include the added subspace for the Y basis
    #   |±i⟩ = 1/√2 · ( |0⟩ ± i·|1⟩ )
    #   |+i⟩⟨+i| = 1/2 · (|0⟩⟨0|+|1⟩⟨1|-i|0⟩⟨1|+i|1⟩⟨0|) = 1/2 · [1 -i, i 1]
    #   |-i⟩⟨-i| = 1/2 · (|0⟩⟨0|+|1⟩⟨1|+i|0⟩⟨1|-i|1⟩⟨0|) = 1/2 · [1 i, -i 1]
    # Also, consequently, we add the new terms to the complete POVM Γ_B

    # Alice's projectors |j⟩⟨j| in C^n_states
    Π_A = [proj(j, n_states) for j in 1:n_states]

    # [CHANGE 11/03/26]
    # Since now we are working in a complex space, we'll declare all the
    # POVM elements as ComplexF64 to avoid problems with Julia (otherwise it
    # won't compile).

    # Bob's POVM elements embedded in C³
    # Z basis: |0⟩⟨0|, |1⟩⟨1| in the qubit subspace
    Γ_0Z = zeros(ComplexF64,dim_B, dim_B); Γ_0Z[1, 1] = 1
    Γ_1Z = zeros(ComplexF64,dim_B, dim_B); Γ_1Z[2, 2] = 1
    # X basis: |±⟩⟨±| in the qubit subspace
    Γ_0X = zeros(ComplexF64,dim_B, dim_B); Γ_0X[1:2, 1:2] = [1 1; 1 1] / 2
    Γ_1X = zeros(ComplexF64,dim_B, dim_B); Γ_1X[1:2, 1:2] = [1 -1; -1 1] / 2
    # Y basis: |+i⟩⟨+i|, |-i⟩⟨-i| in the qubit subspace
    Γ_0Y = zeros(ComplexF64,dim_B, dim_B); Γ_0Y[1:2, 1:2] = [1 -1im; 1im 1] / 2
    Γ_1Y = zeros(ComplexF64,dim_B, dim_B); Γ_1Y[1:2, 1:2] = [1 1im; -1im 1] / 2

    Γ_B = [Γ_0Z, Γ_1Z, Γ_0X, Γ_1X, Γ_0Y, Γ_1Y]
...
function sifting_kraus()
    # [DOUBT 11/03/26]
    # Since we are working with a complex Cone, we must define this matrices as
    # explicitely complex ones: Matrix --> Matrix{ComplexF64}

    project_A = Matrix{ComplexF64}(I, n_dim, n_states)    # C⁴ → C²: keeps |0⟩, |1⟩
    project_B = Matrix{ComplexF64}(I, 2, dim_B)           # C³ → C²: drops |vac⟩
...
function key_map_kraus()
    # [DOUBT 11/03/26]
    # Since we are working with a complex Cone, we must define this matrices as
    # explicitely complex ones.

    Z₀ = kron(ComplexF64.(proj(1, n_dim)), Matrix{ComplexF64}(I, 2, 2)) # Alice bit = 0
    Z₁ = kron(ComplexF64.(proj(2, n_dim)), Matrix{ComplexF64}(I, 2, 2)) # Alice bit = 1
...
# [CHANGE 18/03/26]
# Add the probability of depolarization 'p_dep' into the function.
...
function optimize_eve_information(ϵ::T, η::T, δ::T, p_d::T, p_dep::T) where {T<:Real}
    # [CHANGE 11/03/26]
    # Here we will introduce different modifications, as it's the largest function of the code
    #   1. Turn off the verbosity of Hypatia for cleaner outputs
    #   2. Change the solver to search for complex solutions, as we now work
    #   with complex states, |±i⟩.
    #   3. Change the @constraint() of Y_model and Y_target 

    model = GenericModel{T}(() -> Hypatia.Optimizer{T}(; verbose = false)) # Change "false" to "true" if needed for debugging purposes
    psd_cone, wrapper, hermitian_space = Ket._sdp_parameters(true)  # false -> real-valued and symmetric, true -> complex-valued and hermitian

    # ── Reference states and fidelity matrix ──
    #
    # φ is a 2 × 6 matrix whose columns are the reference states |φⱼ⟩ ∈ C²
    ...
    #
    # With p_{Z_A} = p_{X_A} = p_{Y_A} = 1, all six pⱼ = 1/2 (Eq. D1)
    ...
    if ϵ == 0
        # [CHANGE 18/03/26]
        # In BB84 F == conj(F) but now, since F isn't real, F != conj(F)
        # Therefore, we must change svd(φ) to svd(conj(φ))
        ...
        _, _, v = svd(conj(φ))                              # v is 6 × 2
        V = kron(v, I(dim_B))                               # 18 × 6 isometry
    ...
    # [DOUBT 11/03/26] I MUST CHANGE THIS FOLLOWING WHAT WE DISCUSSED LAST WEEK
    # Since we now have 6 ideal states instead of 4, the constraint is redundant
    # for one yield of two of the basis each (see below).
    #
    # Is it only for the δ = 0? IDK
    #
    # Why?
    #   1. ρ_Z0 + ρ_Z1 = ρ_X0 + ρ_X1 = ρ_Y0 + ρ_Y1 = I/2 (maximally mixed state)
    #   2. Applying Bob's POVM we can get:
    #           Y_{k|Z0} + Y_{k|Z1} = Y_{k|X0} + Y_{k|X1} = Y_{k|Y0} + Y_{k|Y1}
    #   3. If we fix both yields for one basis, we can get the other yields of
    #   the other basis by determining just one of them. E.g.:
    #           Y_{k|X0} = Y_{k|Z0} + Y_{k|Z1} - Y_{k|X1}
    #           Y_{k|Y1} = Y_{k|Z0} + Y_{k|Z1} - Y_{k|Y0}
    #   4. This is related to the fact that the qubit space is C⁴
    #
    # Therefore, now we should constrain Y_{k|j} for, e.g., k ∈ {Z0, Z1, X0, Y0}
    # (columns 1, 2, 3 and 5), although it's not the only possible option
    ...
    @constraint(model, Y_model[:, [1,2,3,5]] .== Y_target[:, [1,2,3,5]])
    ...
    # [DOUBT 11/03/26]
    # It seems like EpiQKDTriCone{T,T} is assuming that the matrix is real and symmetric. Therefore, we must:
    #       EpiQKDTriCone{T,T} --> EpiQKDTriCone{T,Complex{T}}
    # That implies changes in the definitions of the Kraus operators to account for this space change (from R to C).
    #   > Changes in sifting_kraus() and key_map_kraus()

    @constraint(model, [h; ρ_vec] in EpiQKDTriCone{T,Complex{T}}(Ĝ_kraus, ẐĜ_kraus, 1 + length(ρ_vec)))
...
# This is a slightly modified version of the function to be able to process
# different values for p_dep.
...
"""
function secret_key_rate(ϵ::T, δ::T, D, p_dep::T) where {T<:Real}
    # CHANNEL PARAMETERS
    α = T(2) / 10               # fiber loss (dB/km)
    η_d = T(1)                  # detector efficiency (comparison with the paper)
...
#secret_key_rate(0.0, 0.0, 0) # For debugging purposes
...
ϵ_mod = 0.0                               # We're now only interested in the case ϵ = 0
p_dep_mod = [0.0, 0.01, 0.1, 0.15] # To be able to do a comparison bewteen different p_dep values
...
# Dictionary to save the SKR of each p_dep:
...
for p_dep in p_dep_mod # With this loop we go through all the values of p_dep
    println("──────| Line for p_dep = $p_dep |──────")
    ...
    for D in D_mod # With this loop we go trough all the distances D
        ...
        rate = secret_key_rate(ϵ_mod, δ_mod, D, p_dep) # Here we compute the SKR each time
...
for p_dep in sort(collect(keys(results))) # We sort it from smaller values to bigger ones (it's not needed, just for readability purposes)
    column_name = "SKR_$p_dep" # Name of the column
    
    df_export[!, column_name] = results[p_dep] # Insert the complete array from the dictionary as a new column
end

# Save the results into a CSV
CSV.write("SSPvsBB84v2_18-03/epsilon_0/data/SSP_depolarized_data_e0.csv", df_export)
println()
println("SSP data successfully saved into 'SSP_depolarized_data_e0.csv'!")