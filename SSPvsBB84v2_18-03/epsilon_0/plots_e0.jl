...
# Open the data
df_bb84 = CSV.read("SSPvsBB84v2_18-03/epsilon_0/data/BB84_depolarized_data_e0.csv", DataFrame)
df_ssp  = CSV.read("SSPvsBB84v2_18-03/epsilon_0/data/SSP_depolarized_data_e0.csv", DataFrame)

# Extract the x-axis (distance)
D_mod = df_bb84.D_km

# Define the p_dep values we've used for the simulations (to be
# able to access the dictionary's values).
p_dep_mod = [0.0, 0.01, 0.1, 0.15] 
...
plot(
    ...
    title = L"\textrm{\textbf{BB84 vs SSP:~}} \mathbf{\delta = 0}",
    yscale = :log10,
    ylims = (1e-7, 1.0),
    legend = :bottomleft
)
...
for (i, p_dep) in enumerate(p_dep_mod)
    ...
    plot!(D_mod, df_bb84[!, col_name], 
          label=L"BB84 (p_\textrm{dep.} = %$p_dep)", 
          linewidth=1, 
          linestyle=:solid,
          color=colors[i], 
          marker=:none)
    ...
    plot!(D_mod, df_ssp[!, col_name], 
          label=L"SSP (p_\textrm{dep.} = %$p_dep)", 
          linewidth=1, 
          linestyle=:dash,
          color=colors[i], 
          marker=:none)
...
# savefig("./epsilon_0/plots/plot.pdf")